...
![DBeaver main page](images_and_resources/dbeaver_site_main_page.png)
...
홈페이지로 들어가서 다운로드 탭을 누르면 [->홈페이지](https://dbeaver.io/download/)

![DBeaver 커뮤니티 다운로드하기](images_and_resources/dbeaver_site_download_page.png)
...
![DBeaver.dmg](images_and_resources/dbeaver_dmg.png)

개인 컴퓨터라면 오른쪽의 Applications 바로가기에 파일을 드래그해 넣으면 되겠지지만, 교육장의 컴퓨터는 그런 것을 모두 관리자 권한으로 막고 있다. 

![앞으로 자주 보게 될 화면](images_and_resources/bureaucracy_over_ease.png)
관리의 편의성을 위해서는 교육생이 더 열심히 움직여야 한다. ~~그것이 쌈무 정신~~

로컬 설치를 위해서는 우회로가 필요한 것이다.
그 우회로란 사용자 폴더 안에 Applications 폴더를 새로 만들고(관리자 권한을 사용하지 않는), .dmg 등 드래그해서 설치할 수 있는 파일을 그 Applications 폴더에 끌어넣는 것이다.

![우회 설치: 사용자 폴더 내의 Applications 활용](images_and_resources/making_local_Applications_folder.png)
...
![DBeaver 실행화면](images_and_resources/dbeaver_first_image.png)
첫 실행이니만큼 바로 샘플 데이터베이스를 둘러보겠냐는 질문이 나온다. YES를 누르면 바로..  

![드라이버파일_설치_화면](images_and_resources/download_sqlite_driver.png)
